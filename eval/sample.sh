...
OUTPUT="/mnt/workspace/workgroup/moyuan.yty/Samples/samples_${MODEL_NAME}_${CKPT_BASE}"
start=$(date +%s)
DEFAULT_BS=1
...
function run_video_i() {
  # sample, 144p, 9:16, 4s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 4s --resolution 144p --aspect-ratio 9:16 --sample-name sample_4s_144p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
  # sample, 144p, 9:16, 8s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 8s --resolution 144p --aspect-ratio 9:16 --sample-name sample_8s_144p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
  # sample, 144p, 9:16, 16s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 16s --resolution 144p --aspect-ratio 9:16 --sample-name sample_16s_144p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
  # sample, 240p, 9:16, 4s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 4s --resolution 240p --aspect-ratio 9:16 --sample-name sample_4s_240p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
  # sample, 240p, 9:16, 8s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 8s --resolution 240p --aspect-ratio 9:16 --sample-name sample_8s_240p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
  # sample, 240p, 9:16, 16s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 16s --resolution 240p --aspect-ratio 9:16 --sample-name sample_16s_240p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
  # sample, 360p, 9:16, 4s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 4s --resolution 360p --aspect-ratio 9:16 --sample-name sample_4s_360p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
  # sample, 360p, 9:16, 8s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 8s --resolution 360p --aspect-ratio 9:16 --sample-name sample_8s_360p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
  # sample, 360p, 9:16, 16s
  eval $CMD --ckpt-path $CKPT --prompt-path assets/texts/t2v_animal.txt --save-dir $OUTPUT --num-frames 16s --resolution 360p --aspect-ratio 9:16 --sample-name sample_16s_360p_9_16 --batch-size $DEFAULT_BS --start-index $VBENCH_START_INDEX --end-index $VBENCH_END_INDEX 
}
...
for arg in "$@"; do
  ...
  if [[ "$arg" = -2i ]] || [[ "$arg" = --video ]]; then
    echo "Running video samples i..."
    run_video_i
  fi
  # vbench
