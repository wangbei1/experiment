...
def main():
    ...
    if mode == 'DDP':
        ...
        my_auto_wrap_policy2 = functools.partial(
            lambda_auto_wrap_policy,
            lambda_fn=lambda m: m in (list(ema.double_blocks) + list(ema.single_blocks)),
        )
        ...
        my_auto_wrap_policy_text_encoder = functools.partial(
            lambda_auto_wrap_policy,
            lambda_fn=lambda m: m in text_encoder.model.layers,
        )
        
        text_encoder.model = FSDP(text_encoder.model, mixed_precision=fpSixteen, auto_wrap_policy=my_auto_wrap_policy_text_encoder, device_id=torch.cuda.current_device(),
            sharding_strategy=ShardingStrategy.HYBRID_SHARD, use_orig_params=True, device_mesh=device_mesh["rep", "shard"])
    else:
