...
def main():
    ...
    set_seed(cfg.get("seed", 1024) + data_parallel_group.rank()) #调整set_seed为每张卡不同，看下效果
    ...
    if mode == 'DDP':
        ...
        my_auto_wrap_policy = functools.partial(
            ...
            lambda_fn=lambda m: m in list(model.transformer_blocks) + list(model.single_transformer_blocks),
        )
        ema_auto_wrap_policy = functools.partial(
            lambda_auto_wrap_policy,
            lambda_fn=lambda m: m in list(ema.transformer_blocks) + list(ema.single_transformer_blocks),
        )
        
        model = FSDP(model, mixed_precision=fpSixteen, auto_wrap_policy=my_auto_wrap_policy, device_id=torch.cuda.current_device(),
            sharding_strategy=ShardingStrategy.FULL_SHARD, use_orig_params=True) #可调
        ema = FSDP(ema, mixed_precision=fpSixteen, auto_wrap_policy=ema_auto_wrap_policy, device_id=torch.cuda.current_device(),
            sharding_strategy=ShardingStrategy.FULL_SHARD, use_orig_params=True)
        
        text_encoder_auto_wrap_policy = functools.partial(
            lambda_auto_wrap_policy,
            lambda_fn=lambda m: m in [text_encoder.roberta.embeddings, *text_encoder.roberta.encoder.layer, text_encoder.transformation]
        )
        text_encoder = FSDP(text_encoder, auto_wrap_policy=text_encoder_auto_wrap_policy, mixed_precision=fpSixteen, device_id=torch.cuda.current_device())
        text_encoder2_auto_wrap_policy = functools.partial(
            lambda_auto_wrap_policy,
            lambda_fn=lambda m: m in list(text_encoder_2.encoder.block),
        )
        text_encoder_2 = FSDP(text_encoder_2, auto_wrap_policy=text_encoder2_auto_wrap_policy, mixed_precision=fpSixteen, device_id=torch.cuda.current_device(),)
    else:
    ...
    for epoch in range(start_epoch, cfg_epochs):
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                model_args["enable_ffn_only_checkpoint"] = cfg.get("enable_ffn_only_checkpoint", False)
                ...
                with timers["backward"] as backward_t:
                    ...
                    if smoothing_running_loss_avg is None or loss.item() < 1.5 * smoothing_running_loss_avg:
                        ...
                        orig_scale = scaler.get_scale()
                        scaler.update()
                        new_scale = scaler.get_scale()
                        
                        if orig_scale != new_scale:
                            logger.info(f"The grad scale is updated from {orig_scale} to {new_scale}.")
                            if new_scale <= 1:
                                exit(0)
