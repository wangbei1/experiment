...
def get_packed_indices(y_mask, *, lT, lW, lH):
    patch_size = 2
    if lH % patch_size != 0:
        lH += (patch_size - lH % patch_size)
    if lW % patch_size != 0:
        lW += (patch_size - lW % patch_size)
...
def main():
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        #     if ";" in batch_prompts[i]:
        #         batch_prompts_en.append(batch_prompts[i].split(";")[0])
        #         batch_prompts[i] = batch_prompts[i].split(";")[1]
