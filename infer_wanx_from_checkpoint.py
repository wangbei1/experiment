...
def parse_args():
    parser = argparse.ArgumentParser(
        description="单输入单输出 WanX / Video 模型推理：从训练检查点或预训练权重加载并生成视频"
    )
    ...
    ckpt_group.add_argument(
        "--ckpt_path",
        type=str,
        help="预训练权重路径（.safetensors 或 .pth，单文件）"
    )
    ckpt_group.add_argument(
        "--checkpoint_dir",
        type=str,
        help="训练检查点目录（包含 epochX-global_stepY 子目录）"
    )
...
def main():
    ...
    print(f"[模型配置] base_z_dim={base_z_dim} (单分支，单输入单输出)")
    ...
    # ==================================
    # 4. 构建「单输入单输出」模型（16 通道）
    # ==================================
    print("==> 构建单分支 WanX 模型 (in_dim = out_dim = base_z_dim) ...")
    model_cfg = dict(
        type="WanX21",
        patch_size=(1, 2, 2),
        text_len=t5_cfg["text_len"],
        in_dim=base_z_dim,      # ⭐ 单分支：输入就是 16 通道 latent
        dim=1536,
        ffn_dim=8960,
        freq_dim=256,
        text_dim=4096,
        out_dim=base_z_dim,     # ⭐ 输出也是 16 通道 latent 的速度场（rectified flow）
        num_heads=12,
        num_layers=30,
        window_size=(-1, -1),
        qk_norm=True,
    ...
    with torch.no_grad():
        ...
        # 初始噪声 latent：[1, base_z_dim, T', H', W']
        ...
        print("==> 进行 Rectified Flow 采样 (单分支 latent) ...")
        samples = scheduler.sample(
            inference_model,  # 单输入单输出模型
            y,
            z=z,
            prompts=[prompt],
            device=device,
    ...
    # ==================================
    print("==> 解码生成结果 ...")
    latent = samples  # [1, base_z_dim, T', H', W']

    with torch.no_grad():
        video_dec = vae.decode(latent)

    video_rec = video_dec[0] if isinstance(video_dec, (list, tuple)) else video_dec
    print(f"[生成结果] 视频范围: [{video_rec.min().item():.3f}, {video_rec.max().item():.3f}]")
    ...
    video_path = os.path.join(args.output_dir, f"single_{tag}_video.mp4")
