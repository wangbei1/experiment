...
log_every = 1
ckpt_every = 100
outputs = "outputs_videojam_token"

debug_videojam_loss=True
...
scheduler = dict(
    ...
    flow_loss_weight=2.0,   # 以后可以改成 2.0, 5.0 等
    enable_videojam=True,   
    video_loss_weight=1.0,   # ⭐ 先只训 flow
    debug_videojam_loss=True,
)
train_patch_and_head_only = True  # ⭐ 只训 VideoJAM 的 patch_embedding + head