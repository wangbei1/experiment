...
def calculate_flops(model, y, z, prompts, device, additional_args):
    class WrapperModule(torch.nn.Module):
        ...
        def forward(self, x, t, context, seq_len):
            return self.model(x=x, t=t, context=context, seq_len=seq_len)
    
    wrapper_model = WrapperModule(model)
    
    # 使用实际的输入数据
    model_inputs = (
        z,  # x
        torch.zeros(1, device=device, dtype=z.dtype),  # t
        y['context'],  # context
        y['max_seq_len']  # seq_len
    )
    
    try:
        flops = FlopCountAnalysis(wrapper_model, model_inputs)
        return flops
    except Exception as e:
        print(f"Error in FLOPs calculation: {str(e)}")
        return None
...
def main():
    ...
    model.to(torch.cuda.current_device()).to(dtype)
    model.eval()
    ...
    for i in range(0, 1, batch_size):
        ...
        for k in range(num_sample):
            ...
            for loop_i in range(loop):
                ...
                # 在 scheduler.sample() 之前添加
                if cfg.get("measure_flops", False):
                    logger.info('mesureing flops')
