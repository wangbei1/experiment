...
from vidgen.acceleration.communications import all_to_all, split_forward_gather_backward
from vidgen.acceleration.parallel_states import get_sequence_parallel_group
...
class PatchEmbedLinear(nn.Module):
    """Video to Patch Linear Embedding.
    Args:
    ...
    def __init__(
        ...
        self.proj = nn.Linear(np.prod(self.patch_size) * in_chans, embed_dim)
    ...
    def forward(self, x):
        ...
        x = x.reshape(shape=(x.shape[0], x.shape[1], t, self.patch_size[0], h, self.patch_size[1], w, self.patch_size[2]))
        x = torch.einsum('nctohpwq->nthwopqc', x)
        x = x.reshape(shape=(x.shape[0], t * h * w, np.prod(self.patch_size) * x.shape[-1]))
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(
        ...
        # self.proj = nn.Linear(config.in_chans, embed_dim)
        if norm_layer is not None:
            self.norm = norm_layer(embed_dim)
        else:
            self.norm = None
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, T=None, H=None, W=None, xs_mask=None) -> torch.Tensor:
        B, N, C = x.shape
        ...
        if self.qk_norm_legacy:
            ...
            if self.rope:
                if T or (H and W):
                    q, k = self.rotary_emb(q, k, T, H, W, xs_mask)
                else:
                    q = self.rotary_emb(q)
                    k = self.rotary_emb(k)
        ...
        if enable_flash_attn:
            ...
            k = k.permute(0, 2, 1, 3)
            v = v.permute(0, 2, 1, 3)
            # print(q.dtype, k.dtype, v.dtype, self.attn_drop.p, self.scale)
            x = flash_attn_func(
                q,
                k,
                v,
...
class MultiHeadCrossAttention(nn.Module):
    def __init__(self, d_model, num_heads, d_c=None, attn_drop=0.0, proj_drop=0.0):
    ...
    def forward(self, x, cond, mask=None):
        ...
        k = self.k_linear(cond).view(1, -1, self.num_heads, self.head_dim)
        v = self.v_linear(cond).view(1, -1, self.num_heads, self.head_dim)
...
class T2VFinalLayer(nn.Module):
    """
    The final layer of t2v.
    """

    def __init__(self, c_emb_size, hidden_size, num_patch, out_channels, d_t=None, d_s=None):
        super().__init__()
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.linear = nn.Linear(hidden_size, num_patch * out_channels, bias=True)
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            nn.Linear(c_emb_size, 2 * hidden_size, bias=True)
        )
    ...
    def forward(self, x, t, x_mask=None, t0=None, T=None, S=None):
        ...
        if S is None:
            S = self.d_s
        shift, scale = self.adaLN_modulation(t).view(x.shape[0], 2, -1).chunk(2, dim=1)
        x = t2i_modulate(self.norm_final(x), shift, scale)
        if x_mask is not None:
            shift_zero, scale_zero = self.adaLN_modulation(t0).view(x.shape[0], 2, -1).chunk(2, dim=1)
...
class SizeEmbedder(TimestepEmbedder):
    ...
    def forward(self, s, bs, dtype):
        ...
        s_freq = self.timestep_embedding(s, self.frequency_embedding_size).to(dtype)
...
# ===============================================
# 3D RoPE Positional Embedding Functions
# ===============================================
# https://github.com/lucidrains/rotary-embedding-torch


class RoPE3D(RotaryEmbedding):
    def rotate_qk(self, q, k, T, H, W, xs_mask=None):
        if T and H and W:
            freqs = self.get_axial_freqs(T, H, W)
            if xs_mask is not None:
                N, M = q.shape[0:2]
                freqs = rearrange(freqs, "T H W C -> T (H W) C", T=T, H=H, W=W)
                freqs = freqs[None, None,:, :, :].repeat(N, M, 1, 1, 1)
                index = xs_mask["ids_keep"][:, None, :, :, None].repeat(1, M, 1, 1, freqs.shape[4])
                freqs = torch.gather(freqs, dim=3, index=index)
                S = freqs.shape[3]
                freqs = rearrange(freqs, "N M T S C -> N M (T S) C", N=N, M=M, T=T, S=S)
            else:
                freqs = rearrange(freqs, "T H W C -> (T H W) C", T=T, H=H, W=W)
            
        elif not T and H and W:
            freqs = self.get_axial_freqs(H, W)
            freqs = rearrange(freqs, "H W C -> (H W) C", H=H, W=W)
        elif T and not H and not W:
            freqs = self.get_axial_freqs(T)
        else:
            freqs = self.get_axial_freqs(q.shape[-2])
