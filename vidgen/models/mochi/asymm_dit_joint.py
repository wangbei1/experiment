    ...
    def forward(
        ...
        **kwargs
        ...
        if enable_sp:
            ...
            rope_split_dim = 1 if xs_mask is None else 2
            rope_cos = split_x(rope_cos, dim=rope_split_dim)
            rope_sin = split_x(rope_sin, dim=rope_split_dim)
        ...
        if xs_mask is not None:
            ...
            if enable_sp:
                from training_acc.patches.mochi import split_x
                x = split_x(x, dim=1)
                rope_cos_orig = split_x(rope_cos_orig, dim=1)
                rope_sin_orig = split_x(rope_sin_orig, dim=1)
