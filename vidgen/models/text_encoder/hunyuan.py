...
class HunyuanTextEncoder(nn.Module):
    def __init__(
        ...
        max_length: int,
        max_length_video: Optional[int] = None,
        ...
        self.max_length_video = max_length_video if max_length_video is not None else max_length
    ...
    def text2tokens(self, text, data_type="image"):
        ...
        kwargs = dict(
            truncation=True,
            max_length=self.max_length if data_type == "image" else self.max_length_video,
            padding="max_length",
            return_tensors="pt",
        )
