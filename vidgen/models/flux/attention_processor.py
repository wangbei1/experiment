...
class FluxSingleAttnProcessor2_0_FA3:
    r"""
    Processor for implementing scaled dot-product attention (enabled by default if you're using PyTorch 2.0).
    """
    ...
    def __call__(
        ...
        if attn.norm_k is not None:
            key = attn.norm_k(key)
        # xiaoteng：注意，这里为了减少qk的transpose次数，直接把旋转矩阵转置到对应shape
        image_rotary_emb = image_rotary_emb.transpose(1, 2)
        # Apply RoPE if needed
        if image_rotary_emb is not None:
            # YiYi to-do: update uising apply_rotary_emb
            # from ..embeddings import apply_rotary_emb
        ...
        hidden_states, _ = flash_attn_func(query, key, value, causal=False)
...
class FluxAttnProcessor2_0_FA3:
    """Attention processor used typically in processing the SD3-like self-attention projections."""
    ...
    def __call__(
        ...
        # xiaoteng: 注意，这里去掉了冗余的转置，cat时候dim=1
        # attention
