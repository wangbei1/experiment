...
import vidgen.models.mochi.context_parallel as cp
from vidgen.models.vae.mochi_vae.progress import get_new_progress_bar
from vidgen.models.vae.mochi_vae.cp_conv import cp_pass_frames, gather_all_frames
from vidgen.models.vae.mochi_vae.latent_dist import LatentDistribution
import vidgen.models.vae.mochi_vae.cp_conv as cp_conv
    ...
    *,
    ...
    def blend_h(a: torch.Tensor, b: torch.Tensor, blend_extent: int) -> torch.Tensor:
        ...
        for i in range(blend_extent):
            b[:, :, :, :, i] = a[:, :, :, :, -blend_extent + i] * (1 - i / blend_extent) + b[:, :, :, :, i] * (
                i / blend_extent
            )
        return b
    ...
    for i in range(0, H, overlap_height):
        ...
        for j in range(0, W, overlap_width):
            ...
            for k in range(T // frame_batch_size):
                ...
                start_frame = frame_batch_size * k + (0 if k == 0 else -12)
                end_frame = frame_batch_size * (k + 1) + (0 if k != T // frame_batch_size - 1 else remaining_frames)
                tile = x[:, :, start_frame:end_frame]
                tile = encoder(tile)
                if k != 0:
                    tile = tile[:, :, 2:]
...
@torch.inference_mode()
def encode_latents_tiled_temporal(
    encoder,
    x,
    frame_batch_size: int = 48,
    overlap_frames = 12,
    ...
    for k in range(T // frame_batch_size):
        remaining_frames = T % frame_batch_size
        start_frame = frame_batch_size * k + (0 if k == 0 else -overlap_frames)
        end_frame = frame_batch_size * (k + 1) + (0 if k != T // frame_batch_size - 1 else remaining_frames)
        tile = x[:, :, start_frame:end_frame]
        tile = encoder(tile)
        if k != 0:
            tile = tile[:, :, overlap_frames//temporal_stride:]
        temporal.append(tile)
        
    return torch.cat(temporal, dim=2)
