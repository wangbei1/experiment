...
class CogVideoXSafeConv3d(nn.Conv3d):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        min_memory_count = torch.prod(torch.tensor(input[:,:,:self.kernel_size[0]].shape)).item() * 2 / 1024**3
        # Set to 2GB, suitable for CuDNN
        max_memory = max(2, min_memory_count)
        
        # if memory_count > max_memory:
        #     kernel_size = self.kernel_size[0]
        #     part_num = int(memory_count / max_memory)
        #     input_chunks = torch.chunk(input, part_num, dim=2)
        #     print("=====", max_memory, ' ', min_memory_count, ' ', memory_count, ' ', input_chunks[0].shape)
        #     if kernel_size > 1:
...
class CogVideoXCausalConv3d(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if inputs.shape[self.temporal_dim] != 1:
...
def CogVideoXVAE(from_pretrained):
    model = AutoencoderKLCogVideoX.from_pretrained(from_pretrained)
    return model