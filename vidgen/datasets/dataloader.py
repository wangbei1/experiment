...
from .datasets import BatchFeatureDataset, VariableVideoTextDataset, VideoTextDataset, VariableVideoTextWithDurationDataset, VariableVideoTextPerRankDataset
from .sampler import BatchDistributedSampler, StatefulDistributedSampler, VariableVideoBatchSampler, VariableVideoBatchBalanceWithDurationSampler, VariableVideoBatchPerRankSampler
...
def prepare_dataloader(
    ...
    **kwargs,
    ...
    elif isinstance(dataset, VariableVideoTextWithDurationDataset):
        batch_sampler = VariableVideoBatchBalanceWithDurationSampler(
            dataset,
            bucket_config,
            num_replicas=process_group.size(),
            rank=process_group.rank(),
    ...
    elif isinstance(dataset, VariableVideoTextPerRankDataset):
        batch_sampler = VariableVideoBatchPerRankSampler(
            dataset,
            num_replicas=process_group.size(),
            rank=process_group.rank(),
            shuffle=shuffle,
...
def collate_fn_default(batch):
    ...
    if is_enable_sequence_parallel():
        batch_valid = [x["valid"] for x in batch if x is not None]
        if not all(batch_valid):
            batch = [{"valid":x["valid"]} for x in batch if x is not None]
