...
class Bucket:
    ...
    def get_bucket_id(self, T, H, W, frame_interval=1, seed=None):
        resolution = H * W
        approx = 0.8 #？？？？？这里为什么设置个0.8呢，实际训练去掉它

        fail = True
        for hw_id, t_criteria in self.bucket_probs.items():
            if resolution < self.hw_criteria[hw_id] * approx: #这里分辨率是要缩小0.8的，所以实际的原视频分辨率可能比目标桶的分辨率小
                continue
            ...
            for t_id, prob in t_criteria.items():
                ...
                if isinstance(prob, tuple): #如果是tuple的话，第二个概率表示直接从大时长调到小时长进行匹配
                    prob_t = prob[1]
            ...
            if isinstance(prob, tuple):
                prob = prob[0]
            if prob >= 1 or rng.random() < prob: #百分之多少的概率直接保留在当前分辨率的桶，否则就匹配下个分辨率的桶
                fail = False
                break
        if fail:
            return None
...
class SecondsBucket:
    ...
    def get_bucket_id(self, T, H, W, seed=None):
        ...
        fail = True
        for hw_id, t_criteria in self.bucket_probs.items():
            if resolution < self.hw_criteria[hw_id]: #这里分辨率是要缩小0.8的，所以实际的原视频分辨率可能比目标桶的分辨率小
                continue

            # if sample is an image
            if T == 0: #这里0作为图片的s数
                if 0 in t_criteria:
                    rng = np.random.default_rng(seed + self.bucket_id[hw_id][0])
                    if rng.random() < t_criteria[0]:
                        fail = False
                        t_id = 0
                        break
                else:
