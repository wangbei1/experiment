...
def read_file(input_path):
    ...
    elif input_path.endswith(".feather"):
...
def save_sample_imageio(x, save_path=None, fps=8, normalize=True, value_range=(-1, 1), force_video=False, verbose=True):
    """
    Args:
        x (Tensor): shape [C, T, H, W]
    """
    ...
    if not force_video and x.shape[1] == 1:  # T = 1: save as image
        ...
        x = (x * 255).permute(1, 2, 3, 0).type(torch.uint8).cpu()
