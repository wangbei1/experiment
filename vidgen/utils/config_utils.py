...
def parse_args(training=False):
    ...
    parser.add_argument("--exp-dir", default=None, type=str, help="exp dir")
    parser.add_argument("--exp-flag", default=None, type=str, help="exp-flag")
...
def define_experiment_workspace(cfg, get_last_workspace=False):
    ...
    if cfg.get("load", None) is not None:
        if cfg.load == "latest": #支持从最近的checkpoint加载
            assert cfg.exp_flag is not None
            cfg.load = None #如果找不到就默认不load
            directories = sorted(glob(f"{cfg.outputs}/*"), reverse=True)
            for dire in directories:
                if dire.endswith(cfg.exp_flag):
                    files = sorted(glob(f"{dire}/epoch*"), key=lambda x: int(x.split("_step")[-1]), reverse=True)
                    if len(files) > 0:
                        for path in files:
                            if not os.path.exists(os.path.join(path, 'all', '.metadata')):
                                continue
                            cfg.load = path #找到了需要把load的路径传出去
                            exp_name, exp_dir = os.path.basename(dire), dire
                            break
                    break
        elif os.path.exists(cfg.load):
            if cfg.exp_flag in cfg.load:
                exp_name, exp_dir = cfg.load.split('/')[-2], os.path.dirname(cfg.load)
    ...
    if cfg.exp_flag is not None:
        exp_name = f"{exp_name}-{cfg.exp_flag}"
