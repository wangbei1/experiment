...
class FlowDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def training_losses(self,
        ...
        if self.use_timestep_transform:
            t = timestep_transform(
                ...
                fixed_timestep_transform=self.use_fixed_timestep_transform,
            )
        ...
        # ===== 基础 per-pixel MSE（不区分 video/flow）=====
        if weights is None:
            loss_raw = (velocity_pred.float() - (noise.float() - x_start.float())).pow(2)
        else:
            weight = _extract_into_tensor(weights, t, x_start.shape)
            loss_raw = weight * (velocity_pred.float() - (noise.float() - x_start.float())).pow(2)

        # ===== VideoJAM: 32 通道 + 无 xs_mask 时拆分 video/flow =====
        C_total = x_start.shape[1]

        if (C_total == 32) and (xs_mask is None):
            C = C_total // 2  # 16 + 16

            # 直接用 loss_raw 按通道分两半
            loss_video = loss_raw[:, :C]   # [B,16,Tz,Hz,Wz]
            loss_flow  = loss_raw[:, C:]   # [B,16,Tz,Hz,Wz]

            # 各自做 mean_flat -> [B]
            video_loss = mean_flat(loss_video, mask=xt_mask)
            flow_loss  = mean_flat(loss_flow,  mask=xt_mask)

            # 默认 total_loss 先简单相加，外面再用 lambda_video / lambda_flow 重新加权
            loss = video_loss + flow_loss   # [B]

            terms["video_loss"] = video_loss
            terms["flow_loss"]  = flow_loss
            terms["loss"]       = loss
            return terms

        # ===== 非 VideoJAM 或者有 xs_mask 的情况：走原版逻辑 =====
        # 这里直接沿用原始实现，保持兼容性
        loss = loss_raw
        ...
        if unpatchify_loss:
            ...
            xs = xs_mask["mask"].view(B, T * H * W, -1).repeat(1, 1, pt * ph * pw)
            xs = rearrange(
                xs,
                "B (T H W) (pt ph pw) -> B (T pt) (H ph) (W pw)",
                B=B,
                T=T,
                H=H,
                W=W,
            ...
            loss_vec = loss.mean(dim=1).view(-1, Tx * Hx * Wx)
            unmask = 1 - xs if xt_mask is None else (1 - xs) * xt_mask.view(B, Tx, 1, 1)
            unmask = unmask.view(-1, Tx * Hx * Wx)
            rf_loss = (loss_vec * unmask).sum(dim=1) / unmask.sum(dim=1)  # (N)
            terms["rf_loss"] = rf_loss

            pred_x_t = noise + velocity_pred
            mask = xs if xt_mask is None else xs * xt_mask.view(B, Tx, 1, 1)
            mae_loss_value = mae_loss_unpatchify(x_t, pred_x_t, mask)
            terms["mae_loss"] = mae_loss_value
            loss_final = rf_loss + mae_loss_coef * mae_loss_value
            terms["loss"] = loss_final
            return terms
        else:
            patch_size = model.module.config.patch_size
            if isinstance(patch_size, int):
                patch_size = (1, patch_size, patch_size)
            loss_pad = xpadding(loss, patch_size)
            rf_loss = F.avg_pool3d(loss_pad.mean(dim=1), patch_size).flatten(1)  # (N, L)
    ...
    def sample_videojam(
        ...
        """
        多模态（video + flow）采样：token 级 concat 的 VideoJAM 版本。

        参数:
            model:
                WanX21VideoJAM 模型，forward 需支持:
                    - 单模态: out = model(x, t, **arg_c)
                    - 双模态: video_pred, flow_pred = model(x, t, flow=flow_x, **arg_c)

            z: torch.Tensor
                初始噪声 latent，形状 [B, 2C, T, H, W]，
                约定前 C 通道是 video，后 C 通道是 flow。

            model_kwargs: dict
                推理时的条件信息，至少包含:
                    - "context": 条件文本特征
                    - "context_null": 无条件文本特征
            ...
            sample_steps: int
                采样步数（对应 cfg.scheduler.sample_steps）

            sample_shift: float
                噪声 schedule shift（对应 cfg.scheduler.sample_shift）

            guidance_scale: float
                CFG 强度（对应 cfg.scheduler.cfg_scale）

            mode: str
                "t2v" / "i2v"

        返回:
            z: 采样后的 latent, 形状仍为 [B, 2C, T, H, W]
        """
        # ===== 形状检查 =====
        B, C_total, T, H, W = z.shape
        assert C_total % 2 == 0, f"[sample_videojam] expect even channels, got C_total={C_total}"
        C = C_total // 2

        z = z.to(device)

        # ===== 构造采样 sigmas & timesteps（和 RFLOW_WANX21_T2V.sample 一致） =====
        sigma = np.linspace(1.0, 0.0, sample_steps + 1, dtype=np.float32)[:sample_steps]
        sigma = (sample_shift * sigma / (1 + (sample_shift - 1) * sigma))

        # 使用我们自己的 set_timesteps（内部会根据 sigmas 计算 timesteps）
        self.set_timesteps(sigmas=sigma, device=device, shift=1.0)
        timesteps = self.timesteps  # int64, 长度 = sample_steps

        # ===== 构造 cond / uncond 的模型输入参数 =====
        assert mode in ("t2v", "i2v"), f"[sample_videojam] unsupported mode={mode}"

        if mode == "t2v":
            context = model_kwargs["context"]
            context_null = model_kwargs["context_null"]
            ...
            context = model_kwargs["context"]
            context_null = model_kwargs["context_null"]
            clip_feat = model_kwargs["clip_feat"]
            max_seq_len = model_kwargs["max_seq_len"]
            img_mask_latent = model_kwargs["img_mask_latent"]
        ...
        if progress:
            try:
                ...
                timestep_iter = tqdm(timesteps, desc="Flow (video+flow)")
            except Exception:
        ...
        for t in timestep_iter:
            # t: 标量 int64
            timestep = torch.full((B,), int(t), device=device, dtype=torch.int64)

            # 当前 video / flow latent 拆分
            video_latent = z[:, :C]   # [B, C, T, H, W]
            flow_latent  = z[:, C:]   # [B, C, T, H, W]

            # 条件分支
            video_pred_c, flow_pred_c = model(video_latent, t=timestep, flow=flow_latent, **arg_c)
            # 无条件分支
            video_pred_u, flow_pred_u = model(video_latent, t=timestep, flow=flow_latent, **arg_null)

            # 拼回 2C 通道，作为这一时刻的「速度场 / flow_pred」
            noise_pred_cond = torch.cat([video_pred_c, flow_pred_c], dim=1)   # [B, 2C, T, H, W]
            noise_pred_uncond = torch.cat([video_pred_u, flow_pred_u], dim=1)

            # CFG
            model_output = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)

            # 一步 ODE 积分: new z
            z = self.step(model_output, t, z, generator=generator, return_dict=False)[0]
