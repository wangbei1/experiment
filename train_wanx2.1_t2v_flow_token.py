            ...
            for local_step in pbar:
                ...
                with timers["encode_vae"] as encode_vae_t:
                    with torch.no_grad():
                        ...
                        if cfg.get("load_video_features", False):
                            # 已经是 latent 了，直接用
                            z_video = video              # [B, 16, Tz, Hz, Wz]
                            z_flow  = flow
                        else:
                            # 分别 encode video / flow
                            z_video = vae.encode(video)
                            z_video = torch.stack(z_video)   # [B, 16, Tz, Hz, Wz]

                            z_flow  = vae.encode(flow)
                            z_flow  = torch.stack(z_flow)

                        # 用 video latent 作为 mask 生成的参照 shape
                        x = z_video
                ...
                with timers["backward"] as backward_t:
                    if "video_loss" in loss_dict and "flow_loss" in loss_dict:
                        ...
                        # 原来只用 terms["loss"] 的分支，保持不动
                        loss = loss_dict["loss"].mean() if loss_dict["loss"].ndim > 0 else loss_dict["loss"]
                    ...
                    if (
                        ...
                        smoothing_running_loss_avg_dict[s_key] = (
                            ...
                            else smoothing_running_loss_avg_dict[s_key] * 0.9 + loss.item() * 0.1
                        )
                    else:
                        logger.info(
